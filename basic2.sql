...
INSERT INTO employees(employee_id,first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES (206,'William','Gietz','william.gietz@sqltutorial.org','515.123.8181from1994-06-07',1,8300.00,205,11);
...
select * from employees;
select * from employees where department_id in (select department_id from employees group by department_id having count(*)>1) order by department_id, first_name;
...
select num 
case
when num=1 then 'sunday' 
when num=2 then 'monday' 
when num=3 then 'tuesday' 
when num=4 then 'wednesday' 
when num=5 then 'thursday' 
when num=6 then 'friday' 
when num=7 then 'saturday' 
end from test;
------------------------------------------------------------------
...
SELECT 
    ...
    LISTAGG(first_name || ' ' || last_name, ', ') WITHIN GROUP (ORDER BY salary DESC) as employee_names
FROM EMPLOYEE.PUBLIC.employees
GROUP BY department_id;
-----------------------------------------------


select department_id, max(salary), min(salary) from employees where department_id in 
